...
or covariates).
If we use more than one explanatory variable, then we oten call this 
...
(called residuals). This is often also written in vector notation as
$y = X\beta + \epsilon$ where $y$ and $\beta$ are vectors and $X$ is the matrix
with the regressors (called design matrix).
...
Linear regression is done in R using the `lm()` (linear model) function.
