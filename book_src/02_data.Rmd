...
## Data Quality
Get summary statistics for each column to identify outliers and missing values.

```{r }
summary(iris)
```

You can also summarize specific columns using a statistic.

```{r }
iris %>% summarize_if(is.numeric, mean)
```

Another way to inspect your data is to  use a scatterplot matrix (using `ggpairs` from package `GGally`). You can visually identify noise data points and ouliers.
...
To remove NAs and remove duplicates, we often do this:
...
values will also be dropped. Typically, you will spend more time on data cleaning.

## Aggregation

We group the data by species and then calculate a summary statistic for each group.

```{r }
iris %>% group_by(Species) %>% summarize_all(mean)
iris %>% group_by(Species) %>% summarize_all(median)
```
We see what features are different between groups.
...
Sampling 15 rows (flowers) from the tibble. I set the random number generator seed to make the results reproducible.
...
We often use sampling to reduce the size of the data.
