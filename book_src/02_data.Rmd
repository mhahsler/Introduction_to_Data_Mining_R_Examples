...
```{r }
iris_ord <- iris %>% mutate_if(is.numeric, function(x) 
  cut(x, 3, labels = c("short", "medium", "long"), ordered = TRUE))
...
```{r }
iris_ord %>% select(-Species) %>% 
  sapply(xtfrm) %>% cor(method = "kendall")
```
...
```{r }
iris_ord %>% select(-Species) %>% 
  sapply(xtfrm) %>% cor(method = "spearman")
```
