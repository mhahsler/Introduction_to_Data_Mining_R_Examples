...
the correct class label are available. This is why it is called a supervised learning problem.
...
```{r }
Zoo_train |> gain.ratio(type ~ ., data = _) |>
  as_tibble(rownames = "feature") |>
  arrange(desc(attr_importance))
```
...
tree_predator <- Zoo_train_dummy |> rpart(predator ~ ., data = _,
...
```{r }
fit <- Zoo_train |> train(predator ~ type, data = _, method = "rpart",
  control = rpart.control(minsplit = 2),
