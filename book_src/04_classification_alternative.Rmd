...
[Logistic Regression](#appendix_logistic_regression) in the Appendix.
...
Caret uses `nnet::multinom()` which implements penalized multinomial regression.
...
measured against the default class (here mammal).
...
For deep Learning, R offers packages for using [tensorflow](https://tensorflow.rstudio.com/) 
and [Keras](https://keras3.posit.co/).
...
the **class imbalance problem** which is especially problematic when it is
important to identify members of the minority class. In this setting the minority 
class is also often called the positive class since it has the property 
that we want to identify. An example would be that we want to identify the 
patients that are positive for a rare disease.
...
You always need to check if you have a class imbalance problem. Just look at the class
distribution.
...
We see that there is moderate class imbalance with very many mammals and few
amphibians and reptiles in the data set. If we build a classifier, then 
it is very likely that it will not perform well for identifying amphibians 
or reptiles. This effect was seen when building a simple decision tree.
...
The resulting tree has no leaf nodes for these rare animal types meaning that 
it cannot identify them.

Deciding if an animal is a reptile or not is a very class imbalanced problem.
We set up this problem by changing the class variable to make it into a binary
reptile/non-reptile classification problem.
...
By checking the class distribution, we see that classification model is now
highly imbalances with only 5 reptiles vs. 96 non-reptiles.
...
The graph shows that the data is highly imbalanced with encountering a non-reptile 
being about 20 times more likely then encountering a reptile. 
We will try several 
different options in the next few subsections.

We split the data into test and training data so we can evaluate how well the 
classifiers for the different options work. I use here a 50/50 split to make sure
...
measures." means that some test folds used in hyper parameter tuning 
did not contain examples of both
classes. This is very likely with strong class imbalance and small datasets.
...
A popular choice is to balance the class distribution by using [stratified sampling](https://en.wikipedia.org/wiki/Stratified_sampling) with replacement to oversample the
minority/positive class. Other options include [SMOTE](https://en.wikipedia.org/wiki/Synthetic_minority_oversampling_technique) (in package **DMwR**)
or other sampling strategies (e.g., from package **unbalanced**). 

Here, we use stratified sampling with
50 observations per class. **Note:** many samples of the positive/minority class
will be chosen several times.
...
The resulting tree has a reasonable accuracy/kappa on the balanced data.  
However, the real data does not have the distribution of the balanced data, so 
we check the model on the unbalanced testing data.
...
**The accuracy is below the no information rate and
kappa is close to 0!** However,
sensitivity (the
ability to identify reptiles) is now one meaning that the classifier is able to 
identify all reptiles.

We have to make a tradeoff between sensitivity (identifying reptiles) and specificity (how many of the
...
**Note:** We have used a sample ratio of 1:10 meaning that we value identifying
1 reptile as much as misscassifying 10 non-reptiles. This is related to the cost 
usd by the cost-sensitive classifier introduced in Option 4 below.
...
where TP and TN have to be 0. We make FN very expensive with
a cost of 100 compared to the cost for FP of 1. 
...
Unfortunately, most classification algorithms (or their
implementation) do not have the ability to consider misclassification
cost. Cost-sensitivity can be added to ANNs by changing the optimized 
loss function from pure cross-entropy loss to a 
weighted version. Using a custom loss function is possible in most 
...
(black lines) and the difference between the classification probability 
of the two best classes
(color intensity; indifference is 0 shown as white) by evaluating 
the classifier
...
Note: The crazy lines in white areas are an artifact of the visualization. Here the
classifier randomly selects a class.
...
For the default settings for Random forest, the model seems to overfit the 
training data. More data would probably alleviate this issue.
...
The linear SVM (without a kernel) produces straight lines and works well on the iris data.
Most kernels do also well, only the sigmoid kernel seems to find a
very strange decision boundary which indicates that the data does not have a
...
For this simple data set, 2 neurons produce the best classifier. 
The model starts to overfit with 6 or more neurons. I ran the ANN twice with 100 
neurons, the two different decision boundaries indicate that the variability of
the ANN with so many neurons is high.
...
A linear SVM does not work on this data set. An SMV with a radial kernel performs 
well, the other kernels have issues with finding a linear decision boundary in the 
projected space.
...
decisionplot(model, x, class_var = "class", 
  predict_type = c("class", "raw")) + labs(title = "ANN (20 neurons)")
```

The plots show that a network with 4 and 6 neurons performs well, while
